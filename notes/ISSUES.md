...
- 並列モード (`--n-workers > 1`) は `payload_workspace` 制約を有効にすると worker 内で `ValueError: Unknown body: payload_payload_box_mount` で落ちる. worker (`_run_single_restart`) は `build_ur5e_model()` を payload なしで呼ぶため, ペイロード body が存在しないまま `payload_workspace_config` の制約構築が body 名を解決しようとする. sequential は CLI が `build_ur5e_model(payload_xml=...)` で構築したモデルを使うため発生しない. リファクタ前から存在する既存バグ. worker 側の `build_ur5e_model()` に CLI と同じ `payload_xml` を渡すか, worker がメインのモデル構築設定を受け取る必要がある
- 残存 xfail 2 件はプロダクションコードのバグではなくテスト設計の不備. `test_playback_and_estimation_pipeline`: 期待値が"ペイロード単体質量 1.0 kg"だが FT センサーはグリッパー質量 (~0.9 kg) も計測するため推定値 ~2.0 kg が正しい動作であり, 期待値をセンサー先端側全質量に修正するかパイプライン結合テストに徹して質量絶対値検証を除外する必要がある. `test_feasible`: デフォルト制約 + 2 リスタートで `feasible == True` を要求するが最適化の確率的性質を無視しており, 制約緩和, リスタート増, 余裕 > -0.1 への条件緩和のいずれかが必要
