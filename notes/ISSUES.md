...
- ホームポーズが 3 箇所で異なる値を持つ. `optimizer.py` の `UR5E_HOME_QPOS` (`[π/2, -π/2, π/2, -π/2, -π/2, 0]`, `mpc/config.py` が import して使用), `model_builder.py` の `_ARM_HOME_QPOS` (`[1.0316, -1.4615, 2.5621, -4.2422, -1.0316, 0.0002]`), `scenes/tasks/identification.xml` の keyframe arm (`[1.3247, -1.4685, 1.3683, -1.4706, -1.5708, -0.2461]`) がそれぞれ異なる. どれが正か決めて統一する必要がある. 整合性テスト `test_ur5e_home_qpos_matches_xml_keyframe` は skip の状態で存在する
