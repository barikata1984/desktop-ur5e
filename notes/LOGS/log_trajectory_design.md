...
## 2026-06-28 グリッドサーチ 24 条件の詳細結果

site_name バグ修正・ワークスペース再定義・ペイロード設定変更後に 24 条件のグリッドサーチを実施した.
スイープ構成: EE 速度 [0.20, 0.25 m/s] × dq [π/2, π rad/s] × ddq [2π, 4π rad/s²] × ワークスペース半幅 3 設定.

**ベスト条件**

| 条件名 | 条件数 |
|---|---|
| wsx25_y35_ee025_dq1pi_ddq4pi | 6.23 |

全 24 条件が infeasible (ee_velocity 余裕 ≈ -0.0000). 条件数は全条件で有限 (6.2–11.2) となり,
site_name バグ修正の効果を最終確認した.

infeasibility の主要違反制約は EE 速度上限と payload_workspace (ペイロードがワークスペース境界をわずかに超える).
EE 速度上限を引き上げるか, 反復回数を増やすことで実行可能解が得られる可能性がある → ISSUES 参照.
