...
| 15 | `regressor.py:236` + `execution.py:95` pad-to-model-DOF 二重実装 | 根本は arm/full DoF 抽象化の不在 → 修正済 (`DofLayout` に統一, 2026-07-02 P3) |
| 16 | `keyframe.py:41` ほか 20 箇所 `q[:6]` マジックナンバー散在 | 同上 → 修正済 (`DofLayout` に統一, 2026-07-02 P3) |
...
## 2026-07-02 P3 構造リファクタリング (A2, #15-16)

Tier 5 #15, #16 (arm/full DoF を明示する `RobotModel` ラッパーの欠如) に対応する 6 段階のリファクタリングを実施した.

Stage 0: フラグメント XML 内部名を post-attach 後に重複しない形にリネームした (`payload_box_mount` 等).
テストスイートを直書き XML シーンから `build_ur5e_model()` / `build_push_model()` 経由のプログラム的モデル構築に移行した.
死んだ path-1 の `scenes/tasks/*.xml` と `scenes/common/robots/*.xml` を削除し, スナップショットテストの期待値を新しいレグレッサに合わせて再ピン留めした.

Stage 1: `core/names.py` に全 canonical MuJoCo オブジェクト名を定数として集約した.
`core/layout.py` に `DofLayout` (腕関節, グリッパ関節, アクチュエータのインデックス空間を検証付きスライスとしてモデル化するデータクラス) を追加した.
既存コードは変更せず, 純粋な追加のみのステージとした.

Stage 2: 4 箇所に重複していた pad-to-full 実装と約 45 箇所のハードコードされた qpos/ctrl スライスを, 検証済みの `DofLayout` 経由に置き換えた.
腕アクチュエータの書き込みは `set_arm_ctrl`/`hold_gripper_ctrl` を使うようにし, 腕幅 (6), アクチュエータ幅 (nu=7), フル qpos 幅 (nq=14) の暗黙的混同を解消した.

Stage 3: 散在していた MuJoCo 名前文字列リテラルを `core/names.py` の定数に置き換えた.
あわせて F1 (`run_identification.py` の `payload_xml` 未解決), F2 (`PlaybackConfig` の FT センサー名デフォルトが post-attach 名と不一致), F3 (`collision.py` のフォールバック body 名), F4 (`workspace.py` の `preferred_geom` 名), F5 (`RenderConfig` のカメラデフォルト) を修正した.

Stage 4: `OptimizerConfig.body_name`/`site_name`, `PlaybackConfig.body_name`/`site_name`/`ft_site_name`/`force_sensor_name`/`torque_sensor_name`, `MPCConfig.body_name`/`site_name`/`ft_site_name` を削除した.
各消費側は `core/names.py` の定数を直接参照するように変更した.
`optimizer.py` の `UR5E_HOME_QPOS` を削除し, `OptimizerConfig.q0` と `MPCConfig.q0` はデフォルトで `None` のままとした.
呼び出し側がモデルの "home" キーフレームから `DofLayout.arm(data.qpos)` 経由で q0 を導出する設計に統一した.
`test_optimize_deterministic_snapshot` はビット一致を維持した.

Stage 5: production で未使用だった `core/robot.py` (`UR5eRobot`), `core/runner.py` (`SimRunner`), `core/controllers/` (Controller registry) を削除した.
`notes/ISSUES.md` から解消済みの課題 2 件 (高レベル抽象の未使用, ロボット XML の重複コピー) を削除し, ホームポーズ課題のエントリを Stage 4 の結果に合わせて更新した.

全ステージを通じてテストは 148 passed, 10 skipped を維持している.
