...
| # | 場所 |
|---|---|
| 8 | `execution.py:143` PD 制御を有効化した瞬間に broadcast crash |
| 9 | `constraints.py:25` `JointLimits` shape (6,) 固定. `num_joints != 6` で SLSQP crash |
| 10 | `mpc/loop.py:87` `RTLSConfig(n_params=10)` hardcode. FT-offset 拡張で crash |

### Tier 4: silent-wrong だが特殊条件下

| # | 場所 |
|---|---|
| 11 | `mpc/planner.py:96` `except (LinAlgError, ValueError): return 1e12` が shape バグを埋没 |
| 12 | `optimize_trajectory.py:138` `except RuntimeError: pass` で workspace 制約を無警告に無効化 |
| 13 | `io.py:62` `save/load` が非対称. 保存されない config フィールドが 10 個超 |
...
| 17 | `windowed_fourier.py:15` Fourier 実装 3 系統並立 | BaseTrajectory を継承しない |
| 18 | `mpc/config.py:54` dead 定数 `model_path` | どこからも読まれない |

### Tier 6: Hot path の効率損失

| # | 場所 |
|---|---|
| 19 | `mpc/loop.py:150` `np.vstack` 毎ステップ再アロケート (O(K²) メモリ traffic) |
| 20 | `optimizer.py:588` 制約 `fun(x)` を診断のため 2 回評価 |
| 21 | `collision.py:290` trajectory clearance が Python 内側ループ |
| 22 | `sampling.py:65` `mj_name2id` を毎 sample 呼出 |
| 23 | `execution.py:166` `get_site_frame` が毎ステップ名前解決 |
| 24 | `regressor_warp.py:141` `np.linalg.svd` を Python for-loop |

### Tier 7: 保守性 (Simplification / Reuse)

| # | 場所 |
|---|---|
| 25 | `optimizer.py:483` sequential / parallel の bookkeeping 二重 |
| 26 | `optimizer.py:241` objective closure copy-paste |
| 27 | `optimizer.py:312` `margin` / `feasible` / `named_margins` 冗長 |
| 28 | `constraints.py:103` position/velocity/acceleration の 3 制約 factory near-identical |
| 29 | `mpc/planner.py:30` `_QuinticCache` が `_TrajectoryCache` と同構造 |
| 30 | `pushing/mpc.py:220` stick / slide 分岐がほぼ同形 |
| 31 | `fourier_warp.py:146` `_fourier_trajectory_numpy` 重複 |
| 32 | `regressor_warp.py:98` `_batch_skew_numpy` が `_skew` と重複 (計 3 実装) |
| 33 | `regressor.py:24` 手書き `_quat_to_rotation_matrix` |
| 34 | `pushing/task.py:55` 生の `mj_name2id` 呼出 |
| 35 | `optimize_trajectory.py:74` `yaml.safe_load` 重複 |
| 36 | `tests/identification/conftest.py:10` `SCENE_PATH` を独自組み立て |
| 37 | `mpc/loop.py:37` `executed_q` docstring 乖離 |

### Tier 8: テスト品質

| # | 場所 |
|---|---|
| 38 | `test_integration.py:62` 1e12 fallback で自明に緑 (11 とセットで対処) |

### Tier 9: 文書規約違反 (japanese-tech-writing)

| # | 場所 |
|---|---|
| 39 | `notes/PLAN.md:23-31` em ダッシュ用語区切り |
| 40 | `notes/LOGS/log_gpu_rl.md:3` ほか 見出しに em ダッシュ |
| 41 | `notes/LOGS/log_gpu_rl.md:30` 中黒 並列列挙 |
| 42 | `notes/LOGS/log_trajectory_design.md:162` 同上 |
| 43 | `literature/.../Kubus-IROS2007-.../on-line-rigid-object....md:75` em ダッシュ + LLM 口調 |
| 44 | `notes/ISSUES.md:6` 中黒 chain |
