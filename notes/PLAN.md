...
push migration plan (`notes/PLAN_push_model_builder_migration.md`) に従い, pushing/ スクリプト群の `build_ur5e_model()` 移行を完了した.
`ft300s_xml=None` オプションで FT300s なし構成 (push 用) と FT300s あり構成 (同定用) を同一ビルダーで生成する.
